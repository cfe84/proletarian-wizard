...
describe("FileNameAssembler", () => {
  ...
  context("Fixing markdown extension", () => {
    it("adds missing markdown extension", () => {
      // when
      const filename1 = fileNameAssembler.assembleFileName({
        fileName: "2020-01-10 - something",
        path: "",
      });
      // then
      ...
      // when
      const filename2 = fileNameAssembler.assembleFileName({
        fileName: "something",
        path: "",
      });
      // then
    ...
    it("adds missing markdown extension even if there's an .md somewhere in the name", () => {
      // when
      const filename = fileNameAssembler.assembleFileName({
        fileName: "2020-01-10 - something.mdsomewhereinthename",
        path: "",
      });
      // then
      should(filename).endWith(
        "2020-01-10 - something.mdsomewhereinthename.md"
      );
    });
    ...
    it("doesn't add an addition markdown extension when already there", () => {
      // when
      const filename = fileNameAssembler.assembleFileName({
        fileName: "2020-01-10 - something.md",
        path: "",
      });
      // then
  ...
  context("Fixing date prefix in filename", () => {
    ...
    it("does not add date as prefix if it's there", () => {
      const filename = fileNameAssembler.assembleFileName({
        fileName: "2020-01-02 - something.md",
        path: "",
      });
