...
describe("Completion", () => {
  ...
  context("attribute values", () => {
    it("proposes all values for a given attribute", () => {
      ...
      should(completions).deepEqual(
        ctx.parsedFolder.attributeValues["attribute1"]
      );
    });
  ...
  // Deactivating date completion since it doesn't work
  context.skip("date attributes", () => {
    it("proposes today", () => {
      const completions = completion.complete("This @attribute1(today", 22);
      should(completions).eql(DateTime.now().toISODate());
    });
    it("proposes tomorrow", () => {
      const completions = completion.complete("This @attribute1(tomorrow", 25);
      const tomorrow = DateTime.now().plus({ day: 1 });
      should(completions).eql(tomorrow.toISODate());
    });
  });
});
