...
`;
    ...
    // then
    it("Loads folders without quotes", () => {
      should(config.folders.projects).eql("PROJECTS__");
    });
    it("Loads folders with quotes", () => {
      should(config.folders.inbox).eql("INBOX");
    });
    it("Loads folders without quotes and spaces", () => {
      should(config.folders.reference).eql("__ Reference __");
    });
    it("Loads folders with quotes and spaces", () => {
      should(config.folders.archive).eql("The ARCHIVE");
    });
    it("Returns default value for missing folders", () => {
      should(config.folders.recurrences).eql("21 - Recurrence");
    });
  });
});
