...
[d] a delegated todo @assignee(Leah) @anotherBooleanAttr`))
    ...
    it("should load delegated todo from subfolder", () => should(todos).containEql({ status: TodoStatus.Delegated, text: "a delegated todo", file: "ROOT|PROJECTS|2020-01-02 - Something|file2.md", folderType: "projects", project: "2020-01-02 - Something", attributes: { assignee: "Leah", anotherBooleanAttr: true }, line: 9 }))
    it("should load todo with subtasks and project correctly", () => {
      const parent = todos.find(todo => todo.text === "a todo with") as TodoItem
      should(parent).not.be.undefined()
      should(parent.project).eql("this project")
      should((parent.attributes as IDictionary<string>)["project"]).eql("this project")
      should(parent.subtasks).length(3)
      const firstSub = (parent.subtasks as TodoItem[]).find(task => task.text === "a substask of that todo") as TodoItem
      const secondSub = (parent.subtasks as TodoItem[]).find(task => task.text === "another subtask of that todo") as TodoItem
      const thirdSub = (parent.subtasks as TodoItem[]).find(task => task.text === "and another") as TodoItem
      const validateSubCommon = (sub: TodoItem) => {
        should(sub.project).eql("this project")
        should((sub.attributes as IDictionary<string>)["project"]).be.undefined()
      }
      ...
      should((subsub.attributes as IDictionary<string>)["project"]).eql("in another project")
