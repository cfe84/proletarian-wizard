...
[d] a delegated todo`))
    ...
    // then
    it("should load normal todo", () => should(todos).containEql({ status: TodoStatus.Todo, text: "A todo todo", file: ".|file.md" }))
    it("should skip non md files", () => should(todos).not.containEql({ status: TodoStatus.Todo, text: "A todo that should not be loaded", file: ".|file.txt" }))
    it("should load completed todo", () => should(todos).containEql({ status: TodoStatus.Complete, text: "a completed todo", file: ".|file.md" }))
    it("should load in progress todo from subfolder", () => should(todos).containEql({ status: TodoStatus.InProgress, text: "An in progress todo", file: ".|folder|file2.md" }))
    it("should load delegated todo from subfolder", () => should(todos).containEql({ status: TodoStatus.Delegated, text: "a delegated todo", file: ".|folder|file2.md" }))
  })
  context("No content", () => {
    // given
    const rootFolder = "."
    ...
    td.when(deps.fs.lstatSync(".|file.md")).thenReturn({ isDirectory: () => false })
    td.when(deps.fs.readFileSync(".|file.md")).thenReturn(new Buffer(`This is just
    content`))
