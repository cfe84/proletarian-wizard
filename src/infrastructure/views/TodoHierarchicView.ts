...
class TodoTreeItem extends GroupOrTodo {
  ...
  constructor(private todo: TodoItem) {
    ...
    this.description =
      (todo.project || todo.file) + " " + flattenAttributes(todo.attributes);
  ...
  implements vscode.TreeDataProvider<GroupOrTodo>
{
  constructor(private deps: IDependencies, private context: IContext) {
    this._showSelectedOnTop = context.storage
      ...
      : true;
    this._showProjectsOnTop = context.storage
      ? context.storage.get(STORAGEKEY_SHOWPROJECTSONTOP, true)
      : true;
    this._showDatesOnProjects = context.storage
      ? context.storage.get(STORAGEKEY_SHOWDATEONPROJECT, true)
      : true;
    this._showOverdueOnTop = context.storage
      ? context.storage.get(STORAGEKEY_SHOWOVERDUEONTOP, true)
      : true;
    ...
    switch (this._sortBy.sortByOption) {
      ...
      case SortByOption.project:
        todos = todos.sort((a, b) =>
          a.project && b.project
            ? a.project.localeCompare(b.project) * directionMultiplier
            : directionMultiplier
        );
        break;
  ...
  private getProjectsGroup(): Group {
    const projectsFolder = this.deps.path.join(
      this.context.rootFolder,
      this.context.config.folders.projects || ""
    );
    let projects: string[] = [];
  ...
  private getGroupsByProject(): Group[] {
    const getProjects = () =>
      this.context.parsedFolder.todos.reduce(
        (projects: IDictionary<TodoItem[]>, todo: TodoItem) => {
    ...
    return Object.keys(projects)
      .map(
        (project) =>
          new Group(this.formatProjectName(project), projects[project])
      )
