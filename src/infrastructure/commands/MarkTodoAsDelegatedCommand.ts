...
export class MarkTodoAsDelegatedCommand implements ICommand<string | null> {
  constructor(private deps: IDependencies, context: IContext) {
  }
  get Id(): string { return "pw.markTodoAsDelegated" }

  executeAsync = async (): Promise<string | null> => {
    const lineOperations = new LineOperations(this.deps)
    SubstituteLine.substitute((line) => lineOperations.setCheckmark(line, "d"))
    return ""
  }
}