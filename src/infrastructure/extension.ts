...
export function activate(vscontext: vscode.ExtensionContext) {
  ...
  const todoItemFsEventListener = new TodoItemFsEventListener(
    deps,
    context,
    folderParser
  );
  ...
  vscontext.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument((event) =>
      todoItemFsEventListener.onFileSaved(event)
    ),
    vscode.workspace.onDidRenameFiles((event) =>
      todoItemFsEventListener.onFileRenamed(event)
    ),
    vscode.workspace.onDidCreateFiles((event) =>
      todoItemFsEventListener.onFileCreated(event)
    ),
    vscode.workspace.onDidDeleteFiles((event) =>
      todoItemFsEventListener.onFileDeleted(event)
    )
  );
  ...
  const attributeCompletion = new AttributeCompletionItemProvider(
    deps,
    context
  );
  vscontext.subscriptions.push(
